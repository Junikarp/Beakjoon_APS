...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		loop1: for (int k = 0; k < tomato[0][0].length; k++) {
	...
	public static void bfs() {
		...
		while (!queue.isEmpty()) {
			...
			for (int i = 0; i < 6; i++) {
				...
				if (nx >= 0 && ny >= 0 && nz >= 0 && nx < tomato.length && ny < tomato[0].length && nz< tomato[0][0].length && !visited[nx][ny][nz]) {
					visited[nx][ny][nz] = true;
					tomato[nx][ny][nz] = tomato[p.x][p.y][p.z] + 1;
